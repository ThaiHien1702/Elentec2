...
const Button = ({
    ...
    ...props }) => {
        ...
        const sizeClasses = {
            small: 'px-3 py-1 h-8 ext-sm',
            medium: 'px-4 py-2 h-10 text-sm',
            large: 'px-6 py-3 h-12 text-base',
        };
  return (
    <button className={`${baseClasses} ${variantClasses[variant]} ${sizeClasses[size]}`} disabled = {isLoading}{...props}>
        {isLoading ? (<Loader2 className="w-5 h-5 animate-spin"></Loader2>
        ) : (
        <>
        {Icon && <Icon className="w-4 h-4 mr-2"></Icon>}
        {children}
        </>
        )}
    </button>
