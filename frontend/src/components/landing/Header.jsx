...
const Header = () => {
  ...
  return (
       <header className= {`fixed top-0 w-full z-50 transition-all duration-300 bg-gray-100 ${isScrolled ? "bg-white/95 backdrop:backdrop-blur-lg" : "bg-white/0" }`} >
                ...
                <div className="flex items-center justify-between h-16 lg:h-20">
                    ...
                    <div className="hidden lg:flex items-center space-x-4">
                        {isAuthenticated ? (
                            ...
                            <Link to="/signup" className="bg-linear-to-r from-blue-400 to-blue-500 hover:bg-gray-800 text-white px-6 py-2.5 rounded-lg font-medium transition-all duration-200 hover:scale-105 hover:shadow-lg">SignUp</Link>
                        </>)}

                    </div>
                    <div className="lg:hidden">
                        <button onClick={()=> setIsMenuOpen(!isMenOpen)} className="p-2 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-colors duration-200">
                            {isMenOpen ? (<X className="w-6 h-6"></X>) : (<Menu className="w-6 h-6"></Menu>) }
                        </button>
                    </div>
                </div>
            </div>
            {isMenOpen && (
                <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-lg">
                    <div className="px-2 t-2 pb-3 space-y-1 sm:px-3">
                        <a href="#features" className="block py-3 text-gray-600 hover:text-gray-900 hover:bg-gray-50 font-medium transition-colors duration-200">Features</a>
                        <a href="#testimonia" className="block py-3 text-gray-600 hover:text-gray-900 hover:bg-gray-50 font-medium transition-colors duration-200">Testimonia</a>
                        <a href="#faq" className="block py-3 text-gray-600 hover:text-gray-900 hover:bg-gray-50 font-medium transition-colors duration-200">FAQ</a>
                        <div className="border-t border-gray-200 my-2"></div>
                        {isAuthenticated ? (
                            <div className="p-4">
                                <Button onClick={() => navigator("/dashboard")} className="w-full"> 
                                    Go to Dashboard
                                </Button>
                            </div>
                        ) : (
                        ...
                        <Link to="/signup" className="bg-linear-to-r from-blue-400 to-blue-500 hover:bg-gray-800 text-white px-6 py-2.5 rounded-lg font-medium transition-all duration-200 hover:scale-105 hover:shadow-lg">Sign Up</Link>
                        </>)}
                    </div>
                </div>
            )}
